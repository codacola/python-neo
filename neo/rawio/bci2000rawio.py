...
class BCI2000RawIO(BaseRawIO):
    ...
    def _parse_header(self):
        ...
        for chan_ix in range(file_info['SourceCh']):
            ch_name = param_defs['ChannelNames']['value'][chan_ix] if ('ChannelNames' in param_defs and (param_defs['ChannelNames']['value'] is not np.nan) and len(param_defs['ChannelNames']['value']) > chan_ix) else 'ch' + str(chan_ix)
            chan_id = chan_ix + 1
